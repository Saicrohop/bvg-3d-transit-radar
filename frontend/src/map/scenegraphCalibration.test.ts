...
describe('scenegraph calibration fixtures', () => {
  ...
  it('selects a local model and applies the asset-specific Euler correction', () => {
    ...
    expect(getScenegraphOrientation(northboundBus)).toEqual([0, 90, 0])
    expect(getScenegraphOrientation(eastboundTrain)).toEqual([0, 90, 90])
    expect(getScenegraphOrientation(southboundBus)).toEqual([0, -90, 0])
    ...
    expect(getScenegraphTranslation(northboundBus)).toEqual([0, 0, 7.03])
